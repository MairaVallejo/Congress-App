<div class="{{ $class }}">
    <div class="relative">
        <input type="{{ $type }}" id="{{ $id }}" name="{{ $name }}" value="{{ $value }}"
            class="block px-2.5 pb-2.5 pt-5 w-full text-sm text-gray-900 bg-gray-50 @error($name) bg-red-50 @enderror border-0 border-b-2 border-gray-300 @error($name) border-red-300 @enderror appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 @error($name) focus:border-red-600 @enderror peer"
            placeholder=" " />
        <label for="{{ $id }}"
